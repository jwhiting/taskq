...
export function registerTaskTools(server: McpServer, taskStore: TaskStore) {
  ...
  server.registerTool(
    ...
    {
      title: "Checkout Task",
      description: "Checkout a task (by queue name for next available, or by task ID for specific task)",
      inputSchema: {
        queue_or_task_id: z.string().describe("Queue name (for next available task) or task ID (for specific task)"),
  ...
  server.registerTool(
    "list_tasks",
    {
      ...
      inputSchema: {
        ...
        status: z.enum(["pending", "checked_out", "completed", "failed"]).optional().describe("Filter by task status"),
